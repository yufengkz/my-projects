...
export default {
    ...
    methods: {
        ...
        $_renderSlotColumn(h, column) {
            const { prop, label, minWidth = '120', events = {}, align = 'left', field, useSlot, ...rest } = column
                        ...
                        scopedSlots: {
                            default: scope => {
                                if (
                                    ...
                                    !beforeEdit(scope.row, scope.column, scope.row[prop], scope.$index)
                                ) {
        ...
        $_renderToolbarButtons(h) {
            ...
            if (buttons.length > 0) {
                ...
                return this.$_renderButtons(h, buttons, { size: 'mini' }, buttonScope, [selectable ? this.selectRows : this.currentRow], {
        ...
        _validateAll(id, callback) {
            ...
            if (id) {
                ...
                // 校验所有行
                Promise.all(Object.entries(editRowsData).map(([key, data]) => validate(data)))
                    .then(() => {
                        success()
                    })
                    .catch(errors => {
