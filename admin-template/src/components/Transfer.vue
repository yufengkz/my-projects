  ...
  <div class="app-container">
    ...
    >
      <div class="text-right" slot="right-footer" style="padding:4px 10px;">
        <el-button type="primary" class="transfer-footer wide-button footer-btn" icon="el-icon-plus" size="small" @click="addNewInfo()">新增抵押信息</el-button>
      </div>
      <template slot-scope="scope">
        <el-row v-for="(item,index) in scope" :key="index" class="p_father">
          <el-col class="p_son">
            <el-col class="p_b" style="color:#333;">{{typeDetails(item)}}</el-col>
            <el-col v-show="item.twoLevelType === 'equipment'" class="p_b" style="color:#bcbcbc;">发动机编号:{{item.warrantNo}}</el-col>
            <el-col  v-show="item.twoLevelType !== 'equipment'" class="p_b" style="color:#bcbcbc;">地址:{{item.addressName}}{{item.detailAddress}}</el-col>
            <el-col class="p_b" style="color:#bcbcbc;">认定价值:{{item.identifyValue}}万元</el-col>
          </el-col>
          <el-col>
            <el-row class="rows">
              <el-col :span="12" style="color:#333;">权属：{{item.ownerRole$FICDictName}}-{{item.ownerName}}</el-col>
