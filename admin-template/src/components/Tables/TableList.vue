  ...
  <div class="table_list_fix">
    ...
    <div v-if="exportBut && exportBut.length > 0" class="btn-operates">
      <a :key="index" :href="item.href || null" @click="item.method()" v-for="(item, index) in exportBut">
        <el-button type="primary" size="mini" :icon="item.icon" class="wide-button" plain>{{ item.title }}</el-button>
      </a>
    </div>
    <!-- table表格 -->
    <el-table
    ...
    >
      <!-- 复选框 -->
      <el-table-column type="selection" style="width: 55px;" v-if="options && options.selection && (!options.isShow || options.isShow())" />
      ...
      <el-table-column
        ...
        :label="(options && options.labelIndex) || '序号'"
      />
