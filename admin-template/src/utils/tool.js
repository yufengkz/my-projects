...
export function beforeUpload(file, obj) {
  ...
  var option = {
    ...
    error:
      obj.error ||
      '请上传小于20MB的TXT/DOC/DOCX/PDF/JPG/JPEG/GIF/PNG/XLSX/XLS/RAR/ZIP/7Z文件',
...
export function filePreview(documentId, fileName) {
  ...
  let timerId = setInterval(() => {
    if (
      (newWindow.document && newWindow.document.title.includes('priview')) ||
      countDownTime <= 0
    ) {
...
export function fileDownload(documentId) {
  ...
  a.href = `${window.location.origin}${process.env.VUE_APP_BASE_API +
    '/clientplatform/file/download?id='}${documentId}`
...
export function moduleFileDownload(type) {
  ...
  const $body = document.body
  a.href = `${window.location.origin}${process.env.VUE_APP_BASE_API +
    '/clientplatform/file/temp/download?type='}${type}`
  a.style.display = 'none'
  $body.appendChild(a)
  a.click()
  $body.removeChild(a)
...
export function exportFRExcel(param) {
  exportExcelNew(
    '/clientplatform/accountPayable/financierExportAccount',
    param
  )
}
...
export function combineUrlParam(url, params) {
  if (
    toString.call(url) === '[object String]' &&
...
export function isImgOrPdf(filePath) {
  ...
  return (
    ['png', 'jpg', 'jpeg', 'gif', 'tif', 'pdf'].indexOf(ext.toLowerCase()) !==
    -1
  )
}
/**
...
export function isImg(filePath) {
  ...
  return filePath.includes('.png') || filePath.includes('.jpg') || filePath.includes('.jpeg') || filePath.includes('.gif') || filePath.includes('.tif')
  // // 获取最后一个.的位置
  // var index = filePath.lastIndexOf('.')
  // // 获取后缀
  // var ext = filePath.substr(index + 1)
