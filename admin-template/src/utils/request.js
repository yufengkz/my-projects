...
service.interceptors.request.use(
  config => {
    // 对白名单中的接口拦截添加例外
    const inWhiteList = requestWhiteList.every(value =>
      config.url.includes(value)
    )
    ...
    if (!config.url.includes('/clientplatform/file/downloadImgByUrl')) { // 获取图片base64接口不加遮罩层
...
service.interceptors.response.use(
  response => {
    ...
    if (
      ...
      response &&
      !response.config.url.includes('/clientplatform/file/downloadImgByUrl')
    ) {
      loadingRequestCount--
    }
    if (loadingRequestCount === 0) {
