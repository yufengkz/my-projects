<template>
  <el-form ref="form" class="register-content-form step-one" :rules="rules" :model="form">
    ...
    <el-form-item prop="smsVerCode">
      <svg-icon icon-class="验证码-灰色" class="icon"></svg-icon>
      <el-input type="text" v-model="form.smsVerCode" placeholder="请输入手机验证码" maxlength="4" @keyup.enter.native="submitForm"></el-input>
    ...
    return {
      ...
      rules: {
        ...
        smsVerCode: [
          ...
          { min: 4, max: 4, message: '请输入四位手机短信验证码', trigger: 'blur' }
        ]
      }
    }
  },
