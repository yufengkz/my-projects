...
<template>
  <div class="app-container registered-enterprise-detail">
        ...
        <DescriptionList class="m-l-20" :col="24">
          ...
          <Description label="手机号">
            <template slot="content">
              ...
              <el-tag
                v-if="customerInfo.threeStatus === 'Y'"
                type="success"
                class="el-icon-check m-l-15"
                size="mini"
              >运营商三要素验证</el-tag>
    ...
    <el-row class="m-t-30" v-if="customerInfo.faceTime">
        ...
        <DescriptionList class="m-l-20" :col="24">
          <Description label="人脸识别结果">
            <template slot="content">
              ...
              <el-tag
                v-if="customerInfo.faceStatus === 'Y'"
                type="success"
                class="el-icon-check m-l-15"
                size="mini"
              >人脸识别校验</el-tag>
      ...
      <el-col :span="12">
        <ImageUploadDetail v-show="faceAuthFileList && faceAuthFileList.length > 0" :fileList="faceAuthFileList"></ImageUploadDetail>
      </el-col>
    </el-row>
        ...
        <el-col :span="24">
          <DescriptionList class="m-l-20" :col="24">
            <Description label="所属企业">
              <template slot="content">
                <el-button size="small" type="text" @click="goToHandle(item)">{{item.companyName}}</el-button>
                <el-tag v-if="item.companyStatus === 1" type="success" class="el-icon-check m-l-15" size="mini">企业认证</el-tag>
        ...
        <el-col :span="12">
          <DescriptionList class="m-l-20" :col="24">
            <Description v-for="(val, key) in item.companyAuthDetailOutDTOList" :key="key" :label="val.label">
              <template slot="content">
                {{ val.value }}
                <el-tag v-if="val.label === '角色'" type="success" class="el-icon-check m-l-15" size="mini">身份信息认证</el-tag>
              </template>
            </Description>
