<template>
  <div>
    <el-tabs
      type="border-card"
      v-model="curIndex"
      v-show="TableProp.machineryCreateDTOS.length > 0 || TableProp.businessCreateDTOS.length > 0 || TableProp.salaryCreateDTOS.length > 0 || TableProp.plantCreateDTOS.length > 0 || TableProp.workingCreateDTOS.length > 0 || TableProp.breedCreateDTOS.length > 0 || TableProp.subsidiesCreateDTOS.length > 0"
    >
      <!-- 农机服务 -->
      <el-tab-pane
      ...
      >
        <el-table
          ...
          class="m-t-30"
        >
          <el-table-column prop="businessProject" :label="TableLabel.businessLabel.a">
            <template slot-scope="scope">
              {{scope.row.businessProject}}
            </template>
          </el-table-column>
      ...
      >
        <el-table
          ...
          class="m-t-30"
        >
          <el-table-column :label="TableLabel.plantLabel.a">
            <template slot-scope="scope">
          ...
          <!-- <el-table-column v-if="incomeChildType === '99'" prop="otherInfo" :label="TableLabel.plantLabel.a"></el-table-column> -->
      ...
      >
        <el-table
          ...
          class="m-t-30"
        >
          <el-table-column prop="workCompany" :label="TableLabel.fixedWageLabel.a">
            <template slot-scope="scope">
              {{scope.row.workCompany}}
            </template>
          </el-table-column>
...
export default {
  ...
  created() {
    // 在父组件 failyIncomeSave() 方法里也做过数据处理
    // 判断 “this.TableProp.baseDTOS ” 是否已经处理过，如没有，此处再处理
    // const obj = typeof this.TableProp.baseDTOS
    // if (obj === 'object') {
    //   const list = []
    //   for (var key in this.TableProp.baseDTOS) {
    //     list.push(this.TableProp.baseDTOS[key])
    //   }
    //   this.TableProp.baseDTOS = list
    //   console.log(this.TableProp.baseDTOS, '二次处理')
    // }
    this.handle()
  },
  mounted() {},
