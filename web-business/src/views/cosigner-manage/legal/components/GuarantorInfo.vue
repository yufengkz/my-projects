      ...
      <el-col :span="24">
        <div class="titleCont">
          <DescriptionList class="m-l-20" :content="resultData">
            <Description label="机构名称" :style="changeAddressWith ? '1' : '0' ">{{resultData.organName}}</Description>
  ...
  watch: {
    resultData: {
      ...
      handler(newVal, oldVal) {
        ...
        if (newVal.organName && newVal.organName.length > 50) {
          this.changeAddressWith = true
        }
        if (newVal.finances) {
          let obj
