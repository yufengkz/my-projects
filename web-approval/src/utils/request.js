...
import { Loading, Message } from 'fic-element-ui'
...
service.interceptors.response.use(
  response => {
    ...
    if (response.data.code === '401' || response.data.code === 401) {
      window.location.href = `${decodeURI(response.data.redirectUrl)}redirect=${window.location.origin}`
      return false
    }
    // 修改用户角色后 强制用户刷新一下页面 获取到新的权限
    if (+response.data.code === 406) {
      // 修改用户角色后 强制用户刷新一下页面 获取到新的权限
      location.reload()
    }
    // 登录账号异常时退出登录态并跳转到统一登录页面
    if (response.data.status === '9198' || response.data.status === 9198) {
      Message.error(response.data.message || '登录账号异常，请重新登录')
      setTimeout(() => {
        store.dispatch('LogOut').then(redirectUrl => {
          window.location.href = `${decodeURI(redirectUrl)}redirect=${window.location.origin}`
        })
      }, 2000)
      return false
    }
