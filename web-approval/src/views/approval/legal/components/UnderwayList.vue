...
<script>
import { legalAlreadyApprovalListSearch } from '@/api/approval'
...
export default {
  ...
  methods: {
    // 列表查询方法
    async getSourceData() {
      const result = await legalAlreadyApprovalListSearch(this.searchForm)
    ...
    handleSearch(val) {
      ...
      this.searchForm = {
        ...
        socialCreditCode: val.socialCreditCode, // 统一社会信用代码
    ...
    initHandleData(index) {
      ...
      // 获取状态列表
      queryJurisdictionStatus({ businessType: 'C03', completionStatus: '2' }).then(res => {
