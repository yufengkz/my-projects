...
import { creditPendingApprovalListSearch } from '@/api/approval'
...
export default {
  ...
  data() {
    return {
      TableSearchRoleBest: [
        ...
        { label: '审批人', value: 'checkName', type: 'text' }
      ],
      TableSearchRoleOne: [
        { label: '申请编号', value: 'applyNo', type: 'text' },
        { label: '客户姓名', value: 'customerName', type: 'text' },
      ...
      ColumnsBest: [
        ...
        { prop: 'approveAmount', label: '申请额度', align: 'center', ansFormatter: true },
        { prop: 'mapStationName', label: '所属MAP', align: 'center' },
        { prop: 'commitTime', label: '提交时间', align: 'center' },
        { prop: 'submitName', label: '审批人', align: 'center', formatter: this.formentSubmitName },
  ...
  methods: {
    formentSubmitName(row, column, cellValue) {
      return (row.submitName === '-1' || !row.submitName) ? 'MAP中心' : row.submitName
    },
    // 列表查询方法
    async getSourceData() {
      const result = await creditPendingApprovalListSearch(this.searchForm)
