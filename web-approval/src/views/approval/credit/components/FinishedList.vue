...
<script>
import { creditFinishedApprovalListSearch } from '@/api/approval'
...
export default {
  ...
  data() {
    return {
      ...
      ColumnsBest: [
        ...
        { prop: 'approveAmount', label: '授信额度', align: 'center', ansFormatter: true },
  ...
  methods: {
    // 列表查询方法
    async getSourceData() {
      const result = await creditFinishedApprovalListSearch(this.searchForm)
    ...
    initHandleData(index) {
      ...
      // 获取状态列表
      queryJurisdictionStatus({ businessType: 'C01', completionStatus: '3' }).then(res => {
        if (res.status === 200) {
