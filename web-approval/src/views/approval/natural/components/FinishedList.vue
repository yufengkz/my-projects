...
<script>
import { naturalFinishedApprovalListSearch } from '@/api/approval'
...
export default {
  ...
  methods: {
    // 列表查询方法
    async getSourceData() {
      const result = await naturalFinishedApprovalListSearch(this.searchForm)
    ...
    handleShow(row, index) {
      ...
      this.$router.push({ path: '/approval/natural/detail', query: { id: row.id, isShow: 'false', applyNo: row.applyNo } })
    },
    // 通过判断权限id 处理对应的数据
    initHandleData(index) {
      ...
      // 获取状态列表
      queryJurisdictionStatus({ businessType: 'C02', completionStatus: '3' }).then(res => {
        if (res.status === 200) {
