    ...
    >
      <el-form :model="form" status-icon ref="form" :rules="rules" label-position="top" label-width="8px">
        ...
        <el-form-item label="验证码" prop="smsVerCode" scope-slot="error">
          <el-input v-model="form.smsVerCode" autocomplete="off" placeholder="请输入6位验证码" maxlength="6">
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        smsVerCode: [{ required: true, message: '请输入验证码', trigger: 'blur' }, { min: 4, message: '请输入四位验证码', trigger: 'blur' }]
      }
    }
  },
  created() {
