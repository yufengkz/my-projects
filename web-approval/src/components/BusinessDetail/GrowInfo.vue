  ...
  <div>
    ...
    <div v-for="(item, index) in plants" :key="item.id">
      ...
      <div class="m-l-20">
        ...
        <DescriptionList :content="item" class="m-t-10 m-l-10">
          ...
          <Description label="预计种植周期">{{ item.cycleStart }}-{{ item.cycleEnd }}</Description>
          ...
          <Description v-if="item.mechanizedDegree !== '3'" label="自有农机">{{ isYNHandler(item.selfMachine) }}</Description>
          <Description
          ...
          <Description label="是否投农业保险">{{ isYNHandler(item.insuranceFlag) }}</Description>
        </DescriptionList>
    ...
    <div>
      ...
      <DescriptionList :content="creditApplyPlant" class="m-t-10 m-l-10" :col="24">
        <Description label="近三年种植物是否一致">{{ creditApplyPlant.somePlantFlag === 'Y' ? '是' : '否' }}</Description>
        <Description v-for="plant in creditApplyPlant.plantCorps" :key="plant.id" :label="plant.year + '年'">{{ plant.cropsType$FICDictName }}-{{ plant.cropsSpecies$FICDictName }}</Description>
      </DescriptionList>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      productType2columns: [
        {
          prop: 'plantAreaProvince$FICDictName',
          label: '位置',
          align: 'center',
          content: '--',
          formatter (row, column, cellValue) {
            return `${row.plantAreaProvince$FICDictName}-${row.plantAreaCity$FICDictName}-${row.plantAreaCounty$FICDictName}-${row.plantAreaTown$FICDictName}`
          }
        },
        ...
        { prop: 'areaFee', label: '土地流流转费(元/亩/年)', align: 'center', content: '--' },
        {
          ...
          content: '--',
          formatter (row, column, cellValue) {
            return `${row.landRentStart}-${row.landRentEnd}`
          }
        },
        {
          prop: 'applyAreaFlag',
      ...
      columns: [
        ...
        { prop: 'landNatureDes', label: '土地性质', align: 'center', content: '--' },
        { prop: 'ownershipType$FICDictName', label: '种植地权属性质', align: 'center' },
        { prop: 'areaFee', label: '土地流转费(元/亩/年)', align: 'center', content: '--' },
        {
          prop: 'landRentStart',
          label: '地租缴纳期限',
          align: 'center',
  ...
  watch: {
    creditApplyPlant: {
      handler(newVal, oldVal) {
        this.creditApplyPlantHandle(newVal)
      },
      deep: true
    }
  },
  created() {},
  mounted() {
    this.creditApplyPlantHandle(this.creditApplyPlant)
  },
  methods: {
    creditApplyPlantHandle(v) {
      if (v) {
