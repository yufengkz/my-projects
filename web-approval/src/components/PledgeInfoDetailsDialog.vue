      ...
      <div class="content-container">
        <div class="m-t-20" v-if="outCheckSource.twoLevelType === 'receivable_account'">
          ...
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20">
            ...
            <Description label="合同金额">{{outCheckSource.contractAmount ? outCheckSource.contractAmount + '万元' : '--'}}</Description>
            <Description label="应收金额">{{outCheckSource.receivableAmount ? outCheckSource.receivableAmount + '万元' : '--'}}</Description>
            <Description
              label="约定期限"
          ...
          <div class="m-t-20 m-l-20">
            ...
            <el-tabs tab-position="left">
              <el-tab-pane v-for="(item,key) in imageData" :key="key">
                <span slot="label">{{pledgeImage[key]}}</span>
        ...
        <div class="m-t-20" v-else-if="outCheckSource.twoLevelType === 'legal_right'">
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20">
            <Description label="所属权属">{{outCheckSource.ownerRole$FICDictName}}{{outCheckSource.ownerName}}</Description>
            <Description label="企业注册资本金">{{outCheckSource.registerCapital ? outCheckSource.registerCapital + '万元' : '--'}}</Description>
            <Description label="企业资产总额">{{outCheckSource.totalAssets ? outCheckSource.totalAssets + '万元' : '--'}}</Description>
            <Description label="认定价值">{{outCheckSource.identifyValue ? outCheckSource.identifyValue + '万元' : '--'}}</Description>
            <Description label="认定机构">{{outCheckSource.identifyOrgan}}</Description>
            <Description label="股权占比">{{outCheckSource.proportion ? outCheckSource.proportion + '%' : '--'}}</Description>
          </DescriptionList>
          <div class="m-t-20 m-l-20">
