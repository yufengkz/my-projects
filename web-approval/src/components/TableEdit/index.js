...
export default {
  ...
  computed: {
    // 计算表格区域高度
    tableHeight() {
      const { height, innerHeight, pagination, buttons, buttonHeight, paginationHeight } = this
      if (height) {
        let customHeight = height
        if (typeof customHeight === 'string' && customHeight.endsWith('px')) {
  ...
  watch: {
    ...
    editRowsData: {
      handler(newValue) {
        ...
        this.$emit(
          'edit-change',
          this.data.map(item => newValue[item[rowKey]])
        )
      },
      deep: true
    }
  ...
  methods: {
    ...
    $_renderTable(h) {
      ...
      const originTableProps = Object.keys(tableProps).reduce((result, item) => {
      ...
      return (
        <div class='edit-table__container'>
          <Table
            data={data}
            highlightCurrentRow
            rowKey={rowKey}
    ...
    $_getSelectionColumn(h) {
      ...
      return selectable ? [<TableColumn type='selection' width='40' fixed='left' />] : []
    },
    // 获取序号列
    $_getSequenceColumn(h) {
      ...
      if (sequence) {
        return [
          <TableColumn resizable={false} label='序号' width='50' type='index' fixed='left' align='center' index={index => index + 1} />
        ]
      }
      return []
    },
    ...
    $_renderColumns(h, columns) {
      // 整体是否排序
      const sortable = this.sortable ? 'custom' : false
      return columns
        .filter(column => {
          const { hidden } = column
          if (hidden !== undefined) {
          ...
          {...{
            ...
            scopedSlots: {
              default: ({ row, column, $index }) => {
                ...
                return (
                  <Link type='primary' onClick={onClick} class='edit-table__link'>
                    {row[prop]}
                  </Link>
                )
              }
    ...
    $_renderActionColumn(h, column) {
      ...
      return (
        <TableColumn
          ...
          align={align}
          width={width}
          className='edit-table__actioncolumn'
          {...{
            scopedSlots: {
              default: ({ row, column, $index }) => {
    ...
    $_renderSlotColumn(h, column) {
      const { prop, label, minWidth = '120', events = {}, align = 'left', field, ...rest } = column
    ...
    $_renderEditColumn(h, column) {
      ...
      const {
        ...
        align = 'left',
        useSlot = false,
        beforeEdit = (row, column, cellValue, index) => true,
        formatter,
        width
      } = column
      const rules = field.rules || []
      const columnSlotName = typeof useSlot === 'boolean' ? 'column' : `column-${useSlot}`
      ...
      return (
        <TableColumn
          ...
          align={align}
          showOverflowTooltip
          labelClassName={isRequired ? 'edit-table__column--required' : ''}
          {...{
            scopedSlots: {
              default: scope => {
                if (!this.editRowsKey.includes(scope.row[rowKey]) || !beforeEdit(scope.row, scope.column, scope.row[prop], scope.$index)) {
    ...
    $_renderToolbarButtons(h) {
      ...
      // 工具条按钮
      const toolbarBtns = null
      if (buttons.length > 0) {
        const buttonScope = this.$scopedSlots.button
        return this.$_renderButtons(h, buttons, { size: 'small' }, buttonScope, [selectable ? this.selectRows : this.currentRow], {
    ...
    _preActionButtons(actions, ...args) {
       ...
       */
      const analyseFunProp = prop => typeof prop === 'function' ? prop(...args) : prop
      return actions
        .filter(({ before = true }) => analyseFunProp(before))
    ...
    $_renderButtons(h, buttons, props, slot, args, slotArgs) {
      ...
      return newActions.map(btn => {
        ...
        const button = (
          <Button {...{ props: { ...rest, ...props }, directives }} onClick={click}>
            {text}
    ...
    $_renderToolbar(h) {
      ...
      if (buttons || toolbarSlot) {
        return (
          <div class='edit-table__toolbar'>
            <div class='edit-table__buttons'>
              {buttons}
              {/** 可以将自定义内容插入工具条 */}
              {toolbarSlot}
            </div>
    ...
    _validateAll(id, callback) {
      ...
      if (id) {
        ...
        // 校验所有行
        Promise.all(
          Object.entries(editRowsData).map(([key, data]) => validate(data))
        )
          .then(() => {
            success()
          })
  ...
  render(h) {
    ...
    return (
      <div class='edit-table' style={{ height: this.tableContainerHeight }}>
        {toolbar}
        {table}
        {page}
      </div>
