...
<DescriptionList>
  ...
  <Description label="名称">对应的值<tag>标签值</tag></Description>
</DescriptionList>
*/
<template>
  <div class="description-list">
    ...
    <el-row :gutter="gutter">
      <el-col :key="key" v-for="(item, key) in dataSource" v-bind="item.style === '1' ? { xs: 24, sm: 24, md: 24, lg: 24, xl: 24 } : {...gridConfig}">
        ...
        <div class="value">
          {{item.value}}
          <el-tag v-if="item.tag" size="mini" class="m-l-15">{{item.tag}}</el-tag>
        </div>
      </el-col>
    </el-row>
  </div>
...
const handleArrayObj = data => {
  return data.filter(item => item.tag === 'Description').map(item => ({
    label: (item.data && item.data.attrs.label) || '暂无数据',
