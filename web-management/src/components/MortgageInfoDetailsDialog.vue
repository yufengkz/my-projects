      ...
      <div class="content-container">
        <div class="m-t-20" v-if="outCheckSource.twoLevelType === 'house'">
          ...
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20" :col="12">
            ...
            <Description v-if="outCheckSource.hasLease == 'Y'" label="租赁金额:">{{ outCheckSource.rent }}万元</Description>
            <Description v-if="outCheckSource.hasLease == 'Y'" label="租赁期限:">
              {{ outCheckSource.leaseTermStart | formatDate('yyyy-MM-dd') }} 至 {{ outCheckSource.leaseTermEnd | formatDate('yyyy-MM-dd') }}
            </Description>
        ...
        <div class="m-t-20" v-else-if="outCheckSource.twoLevelType === 'special_machine'">
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20" :col="12">
            ...
            <Description label="购买价格">{{ outCheckSource.buyPrice }}万元</Description>
            <Description label="认定价值">{{ outCheckSource.identifyValue }}万元</Description>
            <Description label="认定机构">{{ outCheckSource.identifyOrgan }}</Description>
        ...
        <div class="m-t-20" v-else-if="outCheckSource.twoLevelType === 'agriculture_product'">
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20" :col="12">
            ...
            <Description label="单价">{{ outCheckSource.unitPrice }}元/公斤</Description>
            <Description label="认定价值">{{ outCheckSource.identifyValue }}万元</Description>
            <Description label="认定机构">{{ outCheckSource.identifyOrgan }}</Description>
            <Description label="他项权利证书编号">{{ outCheckSource.otherCertificateNo || '' }}</Description>
            <Description label="房产地址">{{ outCheckSource.addressName }}{{ outCheckSource.detailAddress }}</Description>
        ...
        <div class="m-t-20" v-else-if="outCheckSource.twoLevelType === 'breed'">
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20" :col="12">
            ...
            <Description label="品质">{{ outCheckSource.quality$FICDictName }}</Description>
            <Description label="数量">{{ outCheckSource.number }}头</Description>
            <Description label="他项权利证书编号">{{ outCheckSource.otherCertificateNo || '' }}</Description>
            <Description label="认定价值">{{ outCheckSource.identifyValue }}万元</Description>
            <Description label="认定机构">{{ outCheckSource.identifyOrgan }}</Description>
        ...
        <div class="m-t-20" v-else-if="outCheckSource.twoLevelType === 'land_contract'">
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20" :col="12">
            ...
            <Description label="承包期限:">
              {{ outCheckSource.contractingStart | formatDate('yyyy-MM-dd') }} 至
              {{ outCheckSource.contractingEnd | formatDate('yyyy-MM-dd') }}
            </Description>
            <Description label="权证编号">{{ outCheckSource.warrantNo }}</Description>
            <Description label="他项权利证书编号">{{ outCheckSource.otherCertificateNo || '' }}</Description>
            <Description label="面积">{{ outCheckSource.area }}m²</Description>
            <Description label="认定价值">{{ outCheckSource.identifyValue }}万元</Description>
            <Description label="认定机构">{{ outCheckSource.identifyOrgan }}</Description>
            <Description label="租赁情况">{{ outCheckSource.hasLease | newOldFilter }}</Description>
            <Description v-if="outCheckSource.hasLease == 'Y'" label="租赁金额:">{{ outCheckSource.rent }}万元</Description>
        ...
        <div class="m-t-20" v-else>
          <DescriptionList :content="outCheckSource" class="m-t-20 m-l-20" :col="12">
            ...
            <Description label="树种">{{ outCheckSource.treeSpecies }}</Description>
            <Description label="认定价值">{{ outCheckSource.identifyValue }}万元</Description>
            <Description label="认定机构">{{ outCheckSource.identifyOrgan }}</Description>
            <Description label="地址">{{ outCheckSource.addressName }}{{ outCheckSource.detailAddress }}</Description>
          </DescriptionList>
...
<script>
import { getGuaranteeMortgageFileList } from '@/api/business'
...
export default {
  ...
  methods: {
    ...
    dialogOpen() {
      ...
      getGuaranteeMortgageFileList({ mortgageId: this.outCheckSource.id }).then(res => {
