...
export default {
  ...
  methods: {
    ...
    getOptionNodes() {
      const { options } = this
      return options.map(props => <Option {...{ props }}></Option>)
    }
  },

  render(h) {
    // 此处options作用是从 this.field中排除掉
    // const { prop, options, type, events = {}, clearable = false, ...rest } = this.$attrs
    const { events = {}, clearable = false, ...rest } = this.$attrs
    return (
      <Select
        clearable={clearable}
        v-model={this.val}
